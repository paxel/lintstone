...
### Fix #14: Forced Static Decision Trees and API Simplification (Critical/Performance)

**Issue:** Previously, actors could choose between a static decision tree (via `init`) or dynamic message handling (via `newMessageEvent`). Maintaining both increased code complexity and left a "performance trap" where users might unknowingly use the slower dynamic approach.

**Fix:**
*   Simplified the `LintStoneActor` interface by removing the `init` method.
*   The `newMessageEvent` method is now called exactly once during actor initialization.
*   Actors are now required to use the `inCase`/`otherwise` matching mechanism to define their message handling logic upfront.
*   This change forces the most performant execution path and eliminates the overhead of calling the handler definition logic for every message.

**Impact:**
*   Guaranteed optimal performance for all actors.
*   Cleaner, more focused API.
*   Reduced internal complexity in the `Actor` implementation.

### Fix #15: Decoupled Decision Tree Builder and Optimized Dispatch (High/Architecture)

**Issue:** `StaticMessageAccess` (the internal builder/matcher) inherited from `MessageAccess` (the dynamic matcher), leading to a confusing "interluded" architecture where recording and execution logic were mixed. The dispatch logic also carried unnecessary state from the base class.

**Fix:**
*   Refactored `MessageAccess` from a class into a pure interface in the API.
*   Introduced `DecisionTreeBuilder` in the implementation to handle the recording phase during actor initialization.
*   Introduced a dedicated `StaticDecisionTree` class for optimized message dispatch, completely unrelated to the builder once constructed.
*   Implemented `DynamicMessageAccess` for cases where on-the-fly matching is still required (e.g., inside handlers or tests).
*   Optimized `StaticDecisionTree` to use a fixed-size array of handlers for maximum dispatch performance.

**Impact:**
*   Cleaner API and internal architecture.
*   Reduced memory footprint for registered actors.
*   Improved dispatch performance by eliminating unnecessary state and indirection.

### Fix #16: Resolved Race Conditions in Unit Tests (Medium/Stability)

**Issue:** Certain unit tests (notably `FailingTests` and `ActorLifecycleTest`) exhibited flakiness on CI environments (e.g., GitHub Actions) due to race conditions. These tests used non-thread-safe collections for shared state and relied on fixed `Thread.sleep` calls instead of robust synchronization.

**Fix:**
*   Updated `FailingTests` to use `Collections.synchronizedList` for collecting asynchronous error events.
*   Replaced non-deterministic `Thread.sleep` calls with active waiting loops and `CountDownLatch` where appropriate.
*   Implemented helper methods like `waitForErrorSize` and `waitForAtomicInteger` to ensure tests only proceed once the expected asynchronous state has been reached.

**Impact:** significantly improved test reliability and stability across different environments and execution speeds.

### Fix #17: CI/CD Pipeline Modernization (Low/Stability)

**Issue:** The GitHub Actions workflows were using outdated action versions (v3) and the Oracle JDK distribution, which can be less stable or slower in CI environments. The `maven-surefire-plugin` was also using an old milestone version (3.0.0-M8).

**Fix:**
*   Updated GitHub Actions workflows to use `v4` of `checkout` and `setup-java`.
*   Switched from `oracle` to `temurin` distribution for JDK 21 in CI.
*   Updated `maven-surefire-plugin` to version `3.2.5` in `pom.xml`.
*   Improved Maven command flags in CI for better log readability and snapshot handling.

**Impact:** More reliable and faster CI builds, reducing intermittent failures caused by infrastructure or outdated tooling.
